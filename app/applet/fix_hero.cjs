...
content = content.replace(/useLayoutEffect\s*\(\s*\(\s*\)\s*=>\s*\{[^}]*\},\s*\[\s*\]\s*\);/, originalHeroGsap);

// Replace static hero section
const staticStart = content.indexOf('{/* STATIC HERO SECTION */}');
const showcaseIndex = content.indexOf('<ShowcaseSlider />');

if (staticStart !== -1 && showcaseIndex !== -1) {
  content = content.substring(0, staticStart) + originalHeroMarkup + '\n\n      ' + content.substring(showcaseIndex);
  fs.writeFileSync('src/pages/Home.tsx', content);
  console.log('Hero fully restored successfully via fix_hero.cjs!');
} else {
  console.log('Could not find static hero markers in Home.tsx');
}
