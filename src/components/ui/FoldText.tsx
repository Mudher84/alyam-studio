...
const FoldText = ({
  ...
  const effectiveSplitBy = useMemo(() => {
    if (containsArabic && splitBy === 'char') {
      return 'word';
    }
    return splitBy;
  }, [splitBy, containsArabic]);

  const segments = useMemo(() => {
    let segmentIndex = 0;

    const renderSegment = (content: string, key: string, split: SplitBy = effectiveSplitBy): ReactNode => {
      segmentIndex += 1;
      return (
        <span
    ...
    if (effectiveSplitBy === 'line') {
      return text.split(/(\n|<br\s*\/?>)/i).map((part, index) => {
        if (part === '\n' || /<br\s*\/?>/i.test(part)) return <br key={`br-${index}`} />;
        return (
          <span className="fold-text-line" key={`line-${index}`}>
            {renderSegment(part || '\u00A0', `segment-line-${index}`, 'line')}
          </span>
        );
      });
    }

    if (effectiveSplitBy === 'word') {
      return text.split(/(\s+|<br\s*\/?>)/i).flatMap((part, index) => {
        if (!part) return [];
    ...
    return parts.flatMap((part, pIdx) => {
      if (/<br\s*\/?>/i.test(part)) return [<br key={`br-p-${pIdx}`} />];
      return Array.from(part).map((char, index) => {
        if (char === '\n') return <br key={`br-${pIdx}-${index}`} />;
        return renderSegment(char === ' ' ? '\u00A0' : char, `segment-char-${pIdx}-${index}`);
      });
    });
  }, [text, effectiveSplitBy, hinge, hingeConfig.origin, safePerspective]);
