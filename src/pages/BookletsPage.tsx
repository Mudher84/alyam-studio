...
export default function BookletsPage() {
  ...
  const filteredItems = projects.filter(item => {
    if (item.category !== 'Educational Booklets' && item.category !== 'Booklets' && item.category !== 'Educational Covers') return false;
  ...
  return (
    <div className="min-h-screen bg-[#F6F2EB] text-[#1A1815] flex flex-col selection:bg-amber-200" dir={isRTL ? 'rtl' : 'ltr'}>
      <SEO title={t('edu.title')} description={t('edu.subtitle')} />
      <Navbar />
      ...
      <main className="flex-1 pt-28 sm:pt-32 pb-24 px-4 sm:px-6 md:px-12 lg:px-24">
        ...
        {/* Random Booklet Slideshow Top Section - Educational Stack Variant */}
        <div className="max-w-7xl mx-auto">
          <RandomTopSlider 
            category="Educational Covers" 
            variant="educational"
            badgeText={t('booklets.sliderBadge')}
            headline={t('booklets.sliderHeadline')}
          />
        </div>
        ...
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4 mb-16 pb-6 border-b border-[#E0D7C9]">
          <div className="flex flex-wrap items-center gap-3">
            <button
              onClick={() => openQuoteModal({ defaultCategory: 'booklet', serviceTitle: 'تصميم وتنفيذ ملزمة تعليمية' })}
            ...
            <button
              onClick={() => openAIModal({ defaultTab: 'booklet_summary' })}
            ...
            >
              ...
              <span>{t('booklets.aiSummaryBadge') || 'تلخيص وهيكلة الملازم بالذكاء الاصطناعي'}</span>
            </button>
          </div>
              ...
              >
                ...
                <div className="flex flex-col px-1">
                  ...
                  <p className="text-sm text-[#5A534B] mt-1 font-light line-clamp-1">
                    {item.teacher ? `${t('portfolio.teacherLabel')}: ${getLocalizedField(item, 'teacher', language)}` : (desc || t('edu.subtitle'))}
                  </p>
                  <div className="mt-3 flex items-center gap-2">
                     <span className="text-[10px] font-mono text-amber-700 uppercase tracking-wider px-2 py-0.5 rounded-sm border border-amber-300 bg-amber-100/50">
                       {getLocalizedField(item, 'subject', language) || t('edu.subject')}
                     </span>
                  </div>
                </div>
              </Link>
